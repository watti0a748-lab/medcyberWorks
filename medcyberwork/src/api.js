const BASE =
  ...
  "https://medcyberworks.onrender.com/api";
...
export const api = {
  ...
  submitTask: (id, transcript) =>
    request("POST", `/tasks/${id}/submit`, { transcript }),
  getTaskHistory: () => request("GET", "/tasks/history"),

  // Test
  getTestSample: () => request("GET", "/test/sample"),
  submitTest: (taskId, transcript) =>
    request("POST", "/test/submit", { taskId, transcript }),

  // Earnings
  getEarningsSummary: () => request("GET", "/earnings/summary"),
  getEarningsTransactions: () =>
    request("GET", "/earnings/transactions"),
